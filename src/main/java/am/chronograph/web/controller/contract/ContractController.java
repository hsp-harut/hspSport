...
//    }
	...
	public void init() {
		...
		/*
		 * for(int i = 0; i < 100; i++) { ContractBean contractBean= new ContractBean();
		 * 
		 ...
		 * contractBean.setLastName("Սարգսյան" + i); contractBean.setMiddleName("Ստեփան"
		 * + i);
		 * 
		 * contractBean.setAddress("" + i); contractBean.setWorkingPlace("" + i);
		 * contractBean.setHotel("" + i); contractBean.setNightsNumber(i);
		 * contractBean.setAgent("" + i);
		 * 
		 * contractBean.setAmountOld(i + 1d + ""); contractBean.setAmountNew(i + 2d +
		 * ""); contractBean.setAmountRelax(i + 3d + "");
		 * contractBean.setAmountPersonal(i + 4d + ""); contractBean.setAmountTotal(4 *
		 * i + 10d + "");
		 * 
		 * contractBean.setPeriodicPayment(i + 100d + "");
		 * contractBean.setAmountRemains(i + 50d + "");
		 * 
		 * if(i % 3 == 0) { contractBean.setCopy(true);
		 * contractBean.setStatus(ContractStatus.Overdue); } else if(i % 4 == 0) {
		 * contractBean.setStatus(ContractStatus.Paid); } else if(i % 5 == 0) {
		 * contractBean.setStatus(ContractStatus.Overpaid); }
		 * 
		 * 
		 * contractBean.setStartDate(DateUtil.getDateBeforeAfterDays(new Date(), i,
		 * true)); contractBean.setEndDate(DateUtil.getDateBeforeAfterDays(new Date(),
		 * i, false));
		 * 
		...
		columnsVisibility = Arrays.asList(true, true, true, false, true, true, false, true, false, true, false, false,
	...
	/**
	 * ActionLIstener method called when user clicks on "New contract" button - to
	 * navigate to corresponding UI...
	 */
	 ...
	 * when trying to create/update Contract with already existing bank account
	 * 'Create' or 'Update' clicked - to create new/update process...
	 */
	...
	/**
	 * Change number value cells to Number type (as they exported by default as
	 * text)...
	 * 
	 * @param document
	 */
	...
	private boolean isValidContract() {
		...
		if (StringUtils.isBlank(contractBean.getContractNumber())) {
			...
			noError = false;
		} else if (contractService.existContractByNumberNotById(contractBean.getContractNumber(),
				contractBean.getId())) {
...
//        }
	...
	private boolean isValidBankAccount() {
		 ...
		 * addErrorMessage("contractForm:bankAccountNumber",
		 * "contractValidationBankInvalid");
		 * 
		 * return false; }
		 * 
		 * if(accountNumber.length() != BANK_ACCOUNT_LENGTH_MAP.get(first3chars)) {
		 * addErrorMessage("contractForm:bankAccountNumber",
		 * "contractValidationBankInvalidLength", first3chars,
		 * BANK_ACCOUNT_LENGTH_MAP.get(first3chars));
		 * 
		 * return false; }
		 */
	...
	/**
	 * ActionListener method called when user enables/disables data table filtering
	 * option...
	 */
